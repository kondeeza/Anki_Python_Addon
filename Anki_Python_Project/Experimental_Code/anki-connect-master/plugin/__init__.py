    ...
    def getDueDate(self, cards):
        ...
        for card in cards:
            if self.findCards('cid:{} is:new'.format(card)):
                ...
                if ivl > 0:  # negative ivl is in seconds, positive is in days
                    ivl = abs(ivl)
                else:
                    ivl = ivl * SECONDS_PER_DAY  # convert days ivl to seconds
                due_date = date + ivl
                formatted_due_date = time.ctime(due_date)
                dueDate.append([formatted_due_date, due_date, date, ivl, time.time()])
