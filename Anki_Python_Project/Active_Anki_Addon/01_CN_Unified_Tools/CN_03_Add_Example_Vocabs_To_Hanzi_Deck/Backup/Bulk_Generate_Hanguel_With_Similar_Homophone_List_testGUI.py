...
def BulkGenerateSimilarHanguelWordList(nids):
    ...
    for nid in nids:
        ...
        if ((currentloop/maxLoop)*100 %5 ==0):
...
def createProgressDialog():
    ...
    widget.setWindowTitle("Progress Dialog")
    widget.progress = QProgressBar(widget)
    widget.progress.setGeometry(100, 80, 250, 20)
    widget.show()
    return widget

def updateProgressDialog(widget,v):
    widget.progress.setValue(v)
    widget.show()
